...
assert.ok(indexHtml.includes('renderMediaInfoText(tracks)'), 'Analysis flow must inject exact MediaInfo text output');
...
// Native-flow injection checks + auto advance
assert.ok(indexHtml.includes('async function injectNfoNativeV7(nfoText)'), 'Native NFO injection V7 function missing');
...
assert.ok(indexHtml.includes('requestSubmit'), 'Should submit form when available');
assert.ok(indexHtml.includes('validationTriggered = true;'), 'Validation trigger marker missing');
assert.ok(indexHtml.includes('const injected = await injectNfoNativeV7(nfoText);'), 'Auto injection must run after analysis');
